...
package net.mcreator.rpgskill.keybind;
...
import net.mcreator.rpgskill.procedures.OpenSkilltreeProcedure;
...
public class OpenskillsKeyBinding extends RpgskillModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public OpenskillsKeyBinding(RpgskillModElements instance) {
		super(instance, 33);
	...
	public void initElements() {
		keys = new KeyBinding("key.rpgskill.openskills", GLFW.GLFW_KEY_0, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					RpgskillMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
	...
	private static void pressAction(PlayerEntity entity, int type, int pressedms) {
		...
		if (type == 0) {
			...
			OpenSkilltreeProcedure.executeProcedure(Stream
